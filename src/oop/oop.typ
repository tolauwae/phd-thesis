...
The remainder of this paper is organized as follows: #link(<sec:debugiot>)[2] introduces the challenges in debugging IoT applications. #link(<sec:eboop>)[3] details the main ingredients of event/based out/of/place debugging. Then, #link(<sec:prototype>)[4] presents EDWARD, including both the changes to WARDuino and the debugger frontend in VSC. In #link(<sec:eval>)[5];, we qualitatively and quantitatively evaluate our debugging technique followed by a discussion of related work in #link(<sec:related>)[6];. #link(<sec:conclusion>)[7] concludes the paper.

== Debugging IoT Applications <sec:debugiot>
...
== Event-based Out-of-Place Debugging <sec:eboop>
...
=== Deployment and Remote Debugging <subsec:deployment>
...
=== Capturing a Debug Session <ssec:debugsession>
...
What information constitutes an out-of-place debug session varies per VM and programming model~@marra18. As an example, the following state is captured for a WebAssembly-based VM we used to implement our approach (#link(<subsec:warduino>)[4.1];):
...
=== Accessing Non/transferable Resources <subsec:remoteresources>
...
=== Local Debugging Support <subsec:recordreplay>
...
== EDWARD: an Event/based out/of/place Debugger for WARDuino <sec:prototype>
...
=== Virtual Machine Extensions <subsec:warduino>
...
=== Quantitative Evaluation <subsec:quantative>
...
#figure(
  [#figure(
    ...
    Labeling of Wasm instructions for a smart lamp application (#link(<app:concurrency>)[14] in #link(<subsec:concurrency>)[8];).
  ],
)
<tab:colouring>

===== Code Analysis. <code-analysis.>
To analyze the potential communication needed between debugger and remote device, consider the smart lamp application written in AssemblyScript allowing users to control the brightness of an LED (cf. #link(<app:concurrency>)[14] in #link(<subsec:concurrency>)[8];).

While remote debugging requires network communication between debugger and the remote device for #emph[all] debugging operations and all types of instructions, event/based out/of/place debugging only requires network communication for those instructions that access non/transferable resources. In order to get an estimate of the amount of instructions which are location dependent compared to the non-transferable instruction we labeled each of the Wasm instructions of the smart lamp application’s code as a location-independent instruction, or an instruction that accesses a non/transferable resource. The results shown in #link(<tab:colouring>)[12] confirm our suspicion that location-independent instructions outweigh instructions accessing non/transferable resources.
