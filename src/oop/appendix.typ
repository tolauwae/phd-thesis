...
Apart from the device related bugs discussed in \cref{sec:debugging-common-bug-issues},
software development issues are common in IoT applications~\cite{Amir21}.
Within this type, a common root cause is concurrency faults~\cite{Amir21}.
\Cref{app:concurrency} shows the implementation of a smart lamp application in AssemblyScript.
%In \cref{fig:concurrency}, we provide an extract of an AssemblyScript application
The code allows users to control the brightness of an LED with MQTT messages or two hardware buttons.
%In \cref{fig:concurrency} we show an extract of this application written in AssemblyScript.
%The full code can be found in \cref{app:concurrency}.
...
For this reason, the callback function does not directly change the LED brightness, but it changes the variable \lstinline|delta| to record the requested change.
The function \lstinline|updateBrightness| called in the main application loop changes the actual brightness gradually.
Every time it is called, it changes the brightness by one percent in the direction dictated by the sign of \lstinline|delta|.
After doing this, the absolute value of \lstinline|delta| is lowered by one.
...
The reason is that the second message overwrites the value of the \lstinline!delta! variable before the \lstinline!updateBrightness! function updates the LED.
...
Such concurrency bugs are often time sensitive, and do not always manifest~\cite{McDowell89}.
...
Moreover, the effects of the bug can happen long after the root cause~\cite{perscheid16}, i.e. when the main loop updates the brightness.
...
During this time-traveling debugging session they can then notice that when receiving two messages in a row, the second may overwrite the \lstinline!delta! parameter set by the first message before it was processed by the main loop, revealing the cause of the bug.
Lines 59 and 61 should increase (and decrease) the value of \lstinline!dela! instead of overwriting it.
