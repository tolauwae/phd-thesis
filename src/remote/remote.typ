...
#note[Rust and AssemblyScript programs can be found under the tutorials folder in the #link("https://github.com/TOPLLab/WARDuino")[GitHub repository];, and the benchmarks folder includes C programs.]WARDuino programs can be developed in multiple high-level languages thanks to WebAssembly. Currently, the WARDuino project includes example programs written in Rust, AssemblyScript, and C.
...
/ register-trap: If the table index that the pops from the stack, does not refer to a WebAssembly function with the correct type ($32 times i32 times i32 times i32 arrow.r epsilon.alt$), the instruction will result in a trap as shown in rule #smallcaps[register-trap];.
