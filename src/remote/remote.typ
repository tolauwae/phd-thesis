...
/ Slow Development Cycle.: #block[
    Third, uploading programs to a microcontroller is a slow and tedious process. For every change in the program, however small, the entire program must be recompiled and flashed (uploaded) to the device. This slows down the development cycle as developers need to wait for this process to finish before they can test their programs.
  ]

/ Debuggability.: #block[
    Fourth, debugging facilities are often not available for microcontrollers without (expensive) hardware debuggers. Even then, the debuggers usually only work for the C language. The lack of debugging facilities makes that developers cannot easily inspect the internal state of a microcontroller. They can only observe its external behavior, for example, that an LED that should be blinking, instead remains off. When the device is not behaving as expected, it is difficult to find the root cause of the problem. Many developers resort to printing values to the serial bus to figure out what the device is doing when something goes wrong @makhshari21, but this is very slow and inconvenient.
  ]

/ Hardware Limitations.: #block[
    Fifth, the embedded devices powering IoT applications have severe hardware limitations compared to conventional computer systems. The most important and universal constraints for these devices, are their limited processing power and memory size.
  ]

/ Bare-metal Execution Environments.: #block[
    Finally, due to their heavy constraints, microcontrollers rarely run full-fledged operating systems. Instead, software is run on top of a tiny RTOS, or simply as statically compiled firmware.
  ]

By using higher-level languages, many of these difficulties disappear. The abstractions in these languages can prevent whole classes of bugs and can ease development. Specifically, high-level languages relieve the programmer from manual memory management, and can provide stronger type systems to further avoid mistakes. They also make it easier to support advanced features such as over-the-air updates—where software is updated without flashing via a physical connection—and remote debugging, where a device is debugged through instructions sent from another remote device.

Some high-level languages have been ported to embedded devices using small custom virtual machines @williams14@zerynth-s-r-l-21@nanoframework-contributors21. Unfortunately, these virtual machines only support a subset of the language’s features and only work on a specific range of hardware platforms. A popular example is MicroPython @george21, which is a subset of Python for microcontrollers. For performance reasons access to the peripherals is often baked into high-level programming language. In MicroPython support for displays and sensors is baked into the language and implemented directly in C. As such, if a specific peripheral device is not supported, the language is of limited use. Another issue, is the lack of debuggers for these languages. This is also the case for MicroPython, which has no official debugger, and third party alternatives are very limited. The Mu debugger for example, supports only classic breakpoints and step instructions, and works exclusively on Raspberry Pi devices, which are much more powerful than embedded devices under consideration in this paper. Finally, many high-level languages do not directly support embedded devices at all, as we will discuss further in #link(<remote:related-work>)[\[remote:related-work\]];.

In this work we take a different approach aimed at enabling multiple high-level languages on microcontrollers while mitigating their downsides. To accomplish this goal, we created WARDuino~@gurdeep19, a virtual machine (VM) designed to run WebAssembly~@haas17 on microcontrollers#footnote[WARDuino supports the first release of WebAssembly (MVP).];. Since WebAssembly is a universal compile target, it can enable programs written in a wide variety of languages to run on low-end embedded devices. This is an important design choice to improve the #emph[portability] of our solution. The design of WebAssembly further focuses on a compact representation, since the byte code is intended to be streamable and efficient~@haas17. This compactness is especially important when executing programs within the #emph[hardware limitations] of the embedded devices. Additionally, WebAssembly can achieve performance speeds close to native code~@haas17@jangda19, potentially outperforming other interpreters for high-level languages on microcontrollers.

The WARDuino virtual machine was first presented in 2019 by #cite(<gurdeep19>, form: "prose");, and addressed #emph[the slow development cycle] and the challenging #emph[debuggability] through the initial implementation of a remote debugger with over-the-air reprogramming capabilities. The paper presented these two features as extensions to the operational semantics of WebAssembly, in order to show their interaction and compatibility with the WebAssembly standard, and to ease re-implementation in other virtual machines. Additionally, WARDuino provided support for a limited set of hardware features through WebAssembly functions embedded in the virtual machine. It is necessary to embed this support in the virtual machine, since the #emph[bare-metal execution environments] of embedded devices provide no conventional interfaces to the hardware. Simultaneously, the primitives should be exposed at the level of WebAssembly in order to achieve the highest #emph[portability] possible. However, the paper left some important problems as future work.

First, WebAssembly does not natively support asynchronous code, but many standard M2M protocols for IoT applications such as MQTT rely on asynchronous events. Similarly, IoT applications often rely on asynchronous handling of hardware interrupts. Due to this limitation in WebAssembly, WARDuino lacked any support for either hardware primitives that rely on asynchronicity, or M2M protocols. In this paper, we extend the WebAssembly operational semantics with support for event-driven callback handling. Through this system, callback functions can subscribe to asynchronous events at the WebAssembly level. The functions will be executed whenever such an event occurs. While other proposals for asynchronous code in WebAssembly are being developed @webassembly-community-group22a, these proposals are still in early stages, and often focus heavily on browser applications, making them unsuitable for resource-constrained microcontrollers.

Second, since hardware support is exposed at the WebAssembly level, there is a language barrier that has to be bridged for every higher-level language. The original version of WARDuino did not address this issue, and in practice lacked any real support for high-level languages. In this paper, we show that WARDuino can practically solve the #emph[low-level coding] challenge as promised in the original paper~@gurdeep19. We illustrate how WARDuino can support high-level languages through language symbiosis, by showing different levels of language integration for AssemblyScript, a TypeScript-like language. These examples serve as a general recipe for implementing other language libraries.

Third, the formal rules for the debugger semantics were not used to prove any interesting properties or guarantees for the debugger. In this paper, we improve the semantics for the debugging and over-the-air updates, and provide a proof for observational equivalence between the debugger semantics, and the underlying WebAssembly semantics. This equivalence means that the executions observed by the debugger semantics are precisely the same as those observed by the underlying language semantics.

Fourth, the virtual machine was never used to implement a real-world IoT application, and its evaluation was limited to a comparison of execution speed with only one alternative approach. In this paper, the evaluation of WARDuino has been expanded to include a comparison to another WebAssembly runtime that can run on microcontrollers We also present real-world IoT application written in AssemblyScript and developed using WARDuino.

To further illustrate how WARDuino can provide an improved development experience, closer to conventional programming, we present how WebAssembly enables fast prototyping of emulators, and the improved tool support with the visual debugger plugin for WARDuino in the VS Code IDE. This plugin is an important contribution towards the increased #emph[debuggability] of IoT software in WARDuino. The paper also includes additional code examples that explain how hardware peripherals can be accessed from WebAssembly, as well as a notably improved and expanded presentation of the WARDuino virtual machine architecture.

In summary, our novel contributions compared to the initial paper~@gurdeep19 are:

- A detailed and expanded presentation of the #emph[improved WARDuino VM];: A WebAssembly virtual machine for embedded devices#footnote[The code of our VM is freely available under the Mozilla Public License 2.0 at #link("https://github.com/TOPLLab/WARDuino/");];. (#link(<remote:architecture>)[\[remote:architecture\]];)

- #emph[Support for IoT primitives] (asynchronous hardware peripherals and common M2M protocols) at the WebAssembly level. (#link(<remote:extending>)[\[remote:extending\]] and @primitives)

- A general recipe for supporting WebAssembly-level primitives in high-level languages in the form of #emph[language symbiosis] implemented for the AssemblyScript language, presented through multiple code examples. (#link(<remote:interoperability>)[\[remote:interoperability\]];)

- The first formally described #emph[event-driven callback system] and implementation for handling asynchronous code in WebAssembly. (#link(<remote:formal>)[\[remote:formal\]];)

- The first proof of #emph[observational equivalence] between a debugger semantics and the WebAssembly operational semantics. (#link(<remote:proofsketch>)[\[remote:proofsketch\]];)

- An improved development experience thanks to better tool support through a #emph[visual debugging environment] in VS Code, which currently supports debugging of WebAssembly and AssemblyScript code—and the possibility for fast prototyping of emulators thanks to WebAssembly. (#link(<remote:tools>)[\[remote:tools\]];)

- A smart light application written in AssemblyScript showcasing the new IoT primitives in WARDuino, and demonstrating that the callback system can handle both interrupts from the embedded device itself and from the network via asynchronous communication protocols such as MQTT. (#link(<remote:stability>)[\[remote:stability\]];)

- An expanded comparison of the execution speed of the virtual machine with another WebAssembly runtime that can run on low-end embedded devices. (#link(
    <remote:performance-on-microcontrollers>,
  )[\[remote:performance-on-microcontrollers\]];)
...
Listing~@lst:program contains a minimal example of an MQTT program written in AssemblyScript. MQTT @banks14 is one of the most used M2M protocols @mishra20 for communication in IoT applications. It allows devices to communicate with a large network of other devices via a server, designated as the MQTT broker. The broker accepts topic-based messages from clients and passes these messages on to all clients that have subscribed to these topics. WARDuino provides an MQTT module with all the necessary primitives for the microcontroller to function as an MQTT client.

#figure(caption: [MQTT AssemblyScript program for WARDuino.], supplement: [Listing], kind: "code")[
#code[
```ts
@\label{lst:program:import}@import {delay, MQTT, print, WiFi} from "as-warduino";

@\label{lst:program:until}@function until(attempt: () => void, done: () => boolean): void {
    while (!done()) {
        delay(1000);
        attempt();
    }
...
@\label{lst:program:main}@export function main(): void {
@\label{lst:program:wifi}@    until(() => { WiFi.connect("ssid", "password"); }, WiFi.connected );

    let message = "Connected to wifi network with ip: ";
@\label{lst:program:ip}@    print(message.concat(WiFi.localip()));
@\label{lst:program:init}@    MQTT.init("broker.example.com", 1883);
@\label{lst:program:sub}@    MQTT.subscribe("helloworld", (topic, payload) => { print(payload); });

@\label{lst:program:while}@    while (true) {
@\label{lst:program:check}@        until(() => { MQTT.connect("clientid"); }, MQTT.connected);
        MQTT.poll();
        delay(1000);
    }
}
...
#[
#let imports = [@lst.program:1]
#let until = [@lst.program:3]
#let main = [@lst.program:10]
#let wifi = [@lst.program:11]
#let ip = [@lst.program:14]
#let init = [@lst.program:15]
#let sub = [@lst.program:16]
#let loop = [@lst.program:18]
#let check = [@lst.program:19]


The code in #link(<lst.program>)[\[lst:program\]] starts by importing all necessary WARDuino primitives from the #emph[as-warduino] package for AssemblyScript on line #imports.
...
Our example highlights the goal of the WARDuino project: programming microcontrollers from many high-level language. The code illustrates how developers can use all the features of their high-level language, even those WebAssembly itself does not fully support, such as strings and anonymous functions. Using the WARDuino primitives in high-level languages, does require some glue code behind the scenes. The exact details are discussed in section~@remote:interoperability.
...
#block[
  #box(image("images/architecture.svg", width: 90%))

]
WARDuino allows developers to debug and update the running program over the air. The virtual machine can receive update and debug messages over different channels, such as Wi-Fi or the serial port. Whenever a message arrives, it is put in the debug message queue, where it is visible to the main interpretation loop. During interpretation, the virtual machine will periodically check the queue for new messages and resolve them one at a time as shown in #link(<fig:overview>)[\[fig:overview\]] . Debug messages can instruct the VM to pause, step or resume execution . Update messages can replace the entire WebAssembly module, single functions, or even single variable values .

Importantly, WARDuino contains a number of WebAssembly modules, implementing common libraries and functionality for microcontrollers, such as the `Serial` module or the `HTTP` module . WebAssembly programs can use the primitives from these modules in order to access the hardware of the microcontrollers. Due to the primitives being defined at the level of WebAssembly, it is not always straightforward for developers to use the primitives in their high-level source language. Language specific libraries can enhance the interoperability between a source language and the low-level interfaces of the WebAssembly primitives. #link(<remote:interoperability>)[\[remote:interoperability\]] goes into further detail on supporting high-level languages through language specific libraries.
...
#figure(
  ...
  supplement: [Algorithm],
    pseudocode-list(numbered-title: [Main loop for interpretation in the WARDuino virtual machine.])[$*$
    ...
    + #line-label(<alg.interpretation:while>) *while* done #strong[and] success *do*
      + #line-label(<alg.interpretation:debug>) resolveDebugMessage($s$) $triangle.filled.r$ Can update the running state $s$
      + *if* $s =$ paused *then*
        + #line-label(<alg.interpretation:wait>) awaitDebugMessage() $triangle.filled.r$ Wait until debug queue is not empty
        + *continue* $triangle.filled.r$ Go back to the start of the loop
      + *end if*
      + #line-label(<alg.interpretation:events>) resolveEvent() $triangle.filled.r$ Run callback for event, if any in the queue
      ...
      + #line-label(<alg.interpretation:switch>) *switch* opcode *do*
        ...
        + #line-label(<alg.interpretation:case>) *case* 0x7c ... 0x8a
          + success $arrow.l$ interpretBinaryi64($m$, opcode) $triangle.filled.r$ Perform i64 binary operation
        + ...
      + *end switch*
      + #line-label(<alg.interpretation:next>) m.pc $arrow.l$ m.pc $+ 1$ $triangle.filled.r$ Increment program counter
    + *end while*
    + *if* #strong[not] success *then*
      + #line-label(<alg.interpretation:trap>) #strong[throw] trap $triangle.filled.r$ Check if any operation threw a trap
    + *end if*
])<alg.interpretation>

#link(<alg.interpretation>)[\[alg.interpretation\]] shows the main interpretation loop of WARDuino as pseudocode. WARDuino executes a WebAssembly module $m$, instruction by instruction, in a single loop (#link(<alg.interpretation:while>)[\[alg.interpretation:while\]];). Before any instruction is interpreted, the #emph[resolveDebugMessage] function checks the debug message queue for new incoming messages (#link(<alg.interpretation:debug>)[\[alg.interpretation:debug\]];), resolves the oldest one, and possibly pauses the runtime. If the runtime is not paused, the virtual machine checks the event queue for new asynchronous events, and possibly resolves at most one before starting the actual interpretation. If the runtime is paused however, the virtual machine will go to sleep until a new message arrives in the queue (#link(<alg.interpretation:wait>)[\[alg.interpretation:wait\]];). The #emph[awaitDebugMessage] function does not resolve debug messages, instead the code jumps back to the start of the loop and the debug message is resolved by the #emph[resolveDebugMessage] function. We discuss the debug message and event resolution in more detail in, respectively #link(<remote:debug>)[1.3] and #link(<remote:interrupts>)[1.5];.

For interpreting instructions, the virtual machine keeps track of its own program pointer $m . p c$, which points to the next instruction to be executed in the program buffer. We may dereference this pointer to get the next opcode to execute (#link(<alg.interpretation:opcode>)[\[alg.interpretation:opcode\]];), for example `0x7f`. A switch statement then matches the current opcode (#link(<alg.interpretation:switch>)[\[alg.interpretation:switch\]];). For our example, the switch determines our opcode to be a binary operator for 64-bit integers that will be handled by the #emph[interpretBinaryi64] function. This function resolves the instruction further and returns whether it succeeded. If so, the while-loop continues, and the next opcode is processed. Otherwise, $s u c c e s s$ will become false, and the while loop will stop interpretation.

When the interpretation loop stops due to a failure, the virtual machine will throw the underlying trap (#link(<alg.interpretation:trap>)[\[alg.interpretation:trap\]];)#footnote[We refer interested readers to the paper by #cite(<haas17>, form: "prose") for more information on traps in WebAssembly.];. Alternatively, interpretation halts whenever the end instruction (`0x0b`) of the main entry point is reached. In this case, the #emph[done] variable will be set to `true`, and the main interpretation loop will stop successfully without throwing a trap.
...
#figure(
  kind: "algorithm",
  supplement: [Algorithm],
  pseudocode-list(numbered-title: [Function to interpret operators for 64-bit integers.])[
...
Before events can be resolved, callback functions must be registered for the topics of the events the program expects to receive. Figure~@fig.callbackhandler gives a schematic overview of the callback handling system in the WARDuino virtual machine. When developing our callback handling system, the two most important concerns are:
...
Because microcontrollers often do not have a keyboard or screen, WARDuino provides remote debugging support. This enables developers to set breakpoints and pause or step the execution remotely. Even more, WARDuino allows over-the-air updates of variables and functions as a whole. #link(<remote:tools>)[\[remote:tools\]] contains more details on the tools available for debugging with WARDuino. In the next section we will look further into the features of the WARDuino project that aim to overcome this language barrier.
...
#figure(caption: [AssemblyScript function that calculates the n-th Fibonacci number.], supplement: [Listing], kind: "code")[
#code[
```ts
...
#figure(
  caption: "Blinking LED example in AssemblyScript with the necessary and minimal glue code.", supplement: [Listing], kind: "code",
    grid(
...
As detailed in~\ref{remote:serial-port-communication}, we represent strings as two integers: a start index in the memory and the length of the string.
...
#figure(
  caption: "Example AssemblyScript program with HTTP GET without strings.", supplement: [Listing], kind: "code",
    grid(
        columns: 2,     // 2 means 2 auto-sized columns
        gutter: 1mm,    // space between columns
        [```ts
...
#figure(
  caption: "Example AssemblyScript program with HTTP GET with glue code for strings.", supplement: [Listing], kind: "code",
    grid(
...
#figure(
  caption: "Example AssemblyScript program with HTTP GET with glue code for objects.", supplement: [Listing], kind: "code",
    grid(
        columns: (2fr, 2fr),
        gutter: 1mm,    // space between columns
        [```ts
...
For instance, programs written in C, Rust, and AssemblyScript have been used with WARDuino using the implementation strategies outlined here.#footnote[Example programs can be found on the #link("https://topllab.github.io/WARDuino/")[documentation website] and in the #link("https://github.com/TOPLLab/WARDuino")[GitHub repository]]
...
#figure(
  caption: [#emph[Left]: The $mono("Type")$ struct. #emph[Middle]: A Type specifier for a primitive that takes two 32-bit unsigned integer ($mono("u32")$) and returns nothing. #emph[Right]: The implementation of the $mono("digital_write")$ primitive.], supplement: [Listing], kind: "code",
    grid(
        columns: (1fr, 2fr, 1fr),
        gutter: 1mm,    // space between columns
        [```ts
...
Additionally, it may use the callback system, an example of which will be discussed in section~@remote:implcallback.
...
#figure(
  caption: [Implementation of the $mono("mqtt_init")$ (top) and $mono("mqtt_subscribe")$ (bottom) WARDuino MQTT primitives using the event-based callback handling system.], supplement: [Listing], kind: "code",
)[```cpp
...
Listing~@fig:mqtt-init shows the implementation of the $mono("mqtt\_init")$ primitive on lines 1 to 13.
...
Lines 15-27 of listing~@fig:mqtt-init implement the MQTT subscribe primitive.
...
#figure(
    caption: [#strong[Core debugger semantics.] Small step reduction rules ($arrow.r.hook_{d,i}$) for the WARDuino remote debugger, as extensions to the WebAssembly semantics.],
grid(
    columns: (2fr, 2fr),
    gutter: 1mm,
// Left side
[
  \$\$\\begin{array}{rrcl}
  \\emph{(Debugger State)} & dbg & \\Coloneqq & \\{ rs,msg\_i,msg\_o,s,bp \\} \\\\
  \\emph{(Running State)} & rs & \\Coloneqq & \\textsc{play} \\; | \\; \\textsc{pause} \\\\
  \\emph{(Messages)} & msg & \\Coloneqq & \\emptyset \\; | \\; pause \\; | \\; play \\; | \\; step \\; | \\; dump \\; | \\; break^+ \\; id \\; | \\; break^- \\; id \\\\
  \\end{array}\$\$
    ],
// Right side TODO
[
...
//    {#smallcaps[play];,,,s,bp};v^\*;e^\* \_d,i {#smallcaps[play];,,,s’,bp};v’^\*;e’^\*
//
//    {#smallcaps[pause];,step,,s,bp};v^\*;e^\* \_d,i {#smallcaps[pause];,,,s’,bp};v’^\*;e’^\*
//
//    { #smallcaps[pause];,dump,,s,bp};v^\*;e^\* \_d,i { #smallcaps[pause];,,msg,s,bp };v^\*;e^\*
//
//    {$r s$,pause,,s,bp};v^\*;e^\* \_d,i {#smallcaps[pause];,,,s,bp};v^\*;e^\*
//
//    {#smallcaps[pause];,play,,s,bp};v^\*;e^\* \_d,i {#smallcaps[play];,,,s,bp};v^\*;e^\*
//
//    {rs,(break^+ id),,s,bp};v^\*;e^\* \_d,i {rs,,,s,(bp~~{id}) };v^\*;e^\*
//
//    {rs,(break^- id),,s,bp};v^\*;e^\* \_d,i {rs,,,s,(bp \\{id}) };v^\*;e^\*
//
//    {rs,,,s,bp};v^\*;e^\* \_d,i {#smallcaps[pause];,,,s,bp};v^\*;e^\*
...
At the top of #link(<fig:dbg:syntax>)[\[fig:dbg:syntax\]] we give an overview of our syntactic extensions to the operational semantics of WebAssembly that provide remote debugging support. In the semantics we abstract away the underlying communication primitives, we assume that there is a system in place that reads messages from a stream and places them in the inbox. A concrete implementation may allow communication over the serial port, an HTTP connection or the SPI bus. For ease of exposition all these possible communication methods are modeled through messages $m s g$.

To differentiate the debugger semantics from the underlying language, we write the reduction relation as ($arrow.r.hook_(d , i)$), where $d$ indicates the debugging semantics and $i$ is still the index for the currently executing module. But thanks to how we define the debugger semantics, the operation of a program during debugging is described by the combined reduction rules from the WebAssembly semantics and our debugger semantics.

The semantics of the debugger consists of a state transitioning system where each state consists of a debugger state #emph[dbg];, zero or more local values $v^(\*)$ and a focused operation $e^(\*)$. The main state of the debugger #emph[dbg] is represented as a 5-tuple that holds the running state $r s$, the last incoming message $m s g_i$ the last outgoing message $m s g_o$, the WebAssembly store $s$ and, a set of breakpoints $b p$. The running state indicates whether the virtual machine is paused (#smallcaps[pause];) or running (#smallcaps[play];). Rules for setting $m s g_i$ when messages are received, and for clearing $m s g_o$ when delivering outbound messages are omitted from our semantics as these are dependent on the communication method. The reduction rules for remote debugging are shown in the lower part of #link(<fig:dbg:syntax>)[\[fig:dbg:syntax\]];, we describe them below.

/ vm-run: #block[
    When in the #smallcaps[play] state with no incoming or outgoing messages and no applicable breakpoints, the debugger takes one small step of the small step operational semantics $arrow.r.hook_i$. That is, a regular WebAssembly step is taken.
  ]
...
/ db-step: #block[
    When the debugger receives the $s t e p$ message in the #smallcaps[pause] state, it takes one step ($arrow.r.hook_i$). The debugger remains in the #smallcaps[pause] state.
  ]
...
#block[
  (Observational equivalence). Let $S$ be the WebAssembly configuration ${ s ; v^(\*) ; e^(\*) }$, for which there exists a transition ($arrow.r.hook_i$) to another configuration $S'$ with ${ s' ; v'^(\*) ; e'^(\*) }$. Let the debugging configuration $({ r s , m s g_i , m s g_o , s , b p } ; v^(\*) ; e^(\*))$ with running state $r s$, incoming messages $m s g_i$, outgoing messages $m s g_o$, and set of breakpoints $b p$; be such that processing the stream of incoming message $M^(\*)$ takes exactly one externally visible step (#smallcaps[vm-run] or #smallcaps[db-step];) in the debugger semantic $(arrow.r.hook_e)$, then: $ ({ s ; v^(\*) ; e^(\*) } arrow.r.hook_i { s ' ; v '^(\*) ; e '^(\*) }) arrow.l.r.double ({ r s , m s g_i , m s g_o , s , b p } ; v^(\*) ; e^(\*) arrow.r.hook_e { r s , m s g_i , m s g_o , s ' , b p } ; v '^(\*) ; e '^(\*)) $

]
The left-hand side of the double implication presents a single step in the normal evaluation ($arrow.r.hook_i$) of a WebAssembly program, while the right-hand side presents one or more steps in the debugging semantics ($arrow.r.hook_(d , i)^(\*)$) where only a single step is externally visible ($arrow.r.hook_e$). We will start by sketching the proof for the first implication, that is, an evaluation step in the WebAssembly semantics implies an equivalent series of debugging steps.
...
[
          \$\$\\begin{array}{ l l c l }
  \\emph{(Messages)} & msg & \\Coloneqq & \\dots \\; | \\; upload \\, m^\* \\; | \\; update\_f\~id\_i\~id\_f\~code\_f \\; | \\; update\_l \\, j \\, v \\\\
  \\emph{(Closures)} & cl & \\Coloneqq & \\{\\textsf{inst}\\ i, \\ \\textsf{idx}\\ j, \\ \\textsf{code}\\ f\\}
  \\end{array}\$\$
    ],
// Right side TODO
[
...
//    {#smallcaps[pause];,upload~m^\*,,s,bp};v^\*;e^\* \_d,i {#smallcaps[pause];,,,s’,};v’^\*;e’^\*
//
//    {#smallcaps[pause];,update\_f~id\_i~id\_f~code\_f,,s,bp};v^\*;e^\* \_d,i {#smallcaps[pause];,,,s’,bp};v^\*;e^\*
//
//    {#smallcaps[pause];,update\_l~j~v’,,s,bp};v^j\_1 v v^k\_2;e^\* \_d,i {#smallcaps[pause];,,,s,bp};v^j\_1 v’ v^k\_2;e^\*

#curryst.proof-tree(r)]
))<fig:red_update>

/ upload-m: #block[
    An $u p l o a d$ message instructs WebAssembly to restart execution with a new set of modules $m^(\*)$. We require all these modules to be well typed, $(tack.r m)^(\*)$. The meta-function bootstrap represents WebAssembly’s initialization procedure, described in the original WebAssembly paper @haas17. Note that this procedure replaces the entire configuration, including the WebAssembly state, locals and stack. Furthermore, upon receiving the $u p l o a d$ message the debugger state is reset and all breakpoints removed.
  ]
...
[
  \$\$\\begin{array}{lrcl}
  (store) & s & \\Coloneqq & \\{\\dots , \\textsf{status}\\,rs,\\,\\textsf{evt}\\ evt^\*,\\ \\textsf{cbs}\\ cbs\\} \\\\
  (running\\ state) & rs & \\Coloneqq & \\textsc{play}\\,|\\,\\textsc{pause}\\,|\\,\\textsc{callback} \\\\
  \\\\
  (event) & evt & \\Coloneqq & \\{\\textsf{topic}\\ memslice,\\ \\textsf{payload}\\ memslice\\} \\\\
  (memory\\ slice) & memslice & \\Coloneqq & \\{ \\textsf{start} \\, \\textit{i32}, \\textsf{length} \\, \\textit{i32} \\} \\\\

  \\\\
  (callback\\ map) & cbs\[x \\rightarrow f\] & \\Coloneqq & \\lambda y.\\textsf{if}\\ y = x\\ \\textsf{then}\\ f\\ \\textsf{else}\\ cbs\\ y \\\\ \\\\

  (instructions) & e & \\Coloneqq & \\dots\\,|\\,\\key{event.push}\\,|\\,\\key{callback}\\{e^\*\\}\\,e^\*\\key{end}\\,|\\,\\key{callback.set}\\,\\textit{memslice}\\,| \\\\
  & & & \\,\\key{callback.get}\\,\\textit{memslice}\\,|\\,\\key{callback.drop}\\,\\textit{memslice} \\\\
  \\end{array}\$\$
    ],
// Right side TODO
[
...
#block[
  #block[
    ...
    s;v^\*;( topic) \_i s;v^\*;s\_cbs(topic)

    s;v^\*;(#emph[topic];)(#emph[payload];)() \_i s’;v^\*;
...
/ skip-message: #block[
    When no callback function is registered in the callback mapping $s_(c b s)$ for the event at the top of the FIFO event queue $s_(e v t) (0)$, the skip-message rule takes a step by removing the top event from the event queue in the store.
  ]
...
We end the section by looking at WARDuino's conformity to the WebAssembly standard (section~@remote:comformance-to-the-wa-standard).
...
Espruino @williams14 is a commercial JavaScript based microcontroller platform for IoT applications.
...
The pins of the device are exposed as global JavaScript objects with methods for adjusting their value, \texttt{D14.set()} for example makes pin D14 high.
Other features, such as Wi-Fi connectivity, can be imported and present themselves as JavaScript objects as well.

WASM3 @massey21 is a fast WebAssembly interpreter, which uses a special compilation technique rather than JIT compilation to achieve good performance.
It has explicit support for microcontrollers such as the ESP32 @espressif-systems23a.
...
The WebAssembly code was generated from C code with Clang 13 \cite{clang-contributors21}. %\footnote{\url{https://releases.llvm.org/13.0.0/tools/clang/docs/ReleaseNotes.html}}
...
In fact, the measurements were performed on an ESP32-DevKitC V4 board\footnote{\url{https://docs.espressif.com/projects/esp-idf/en/latest/esp32/hw-reference/esp32/get-started-devkitc.html}}.
...
There exist more resource-rich devices that are used for IoT applications, such as the Raspberry Pi devices, but these are so powerful that many of the challenges outlined in this paper are present to a far lesser extend.
...
A very recent paper by #cite(<phipps-costin23>, form: "prose");, alternatively proposes a universal target for non-local control flow that relies on effect handlers @plotkin09. In our opinion this solution is more attractive than the threads and stack switching proposal, primarily due to its simplicity—it only adds three new instructions—and its universality. Similar to the stack switching proposal, a callback handling system comparable to the one described here, could most likely be built on top of this system. However, continuations are still expensive, since they also need to save the entire stack. Furthermore, the proposal is again not enough to support asynchronous primitives. The effect handlers would only allow us to create a system for handling interrupts with callback functions, directly in WebAssembly code. In this case, we arrive at the same solution we have outlined in this paper, except the implementation has moved from the virtual machine to WebAssembly code. It is not clear whether this approach would have any benefits.

The WebAssembly System Interface (WASI) @hickey20 is a collection of standardized APIs for system level interfaces. It is not part of the official WebAssembly standard, but is widely used. The WASI `pthreads` API could be used to implement interrupts in WebAssembly. WASI provides a means to access system level APIs in WebAssembly. As with our approach, these API functions can be imported from a module named `env`. The expectation is that the WASI `pthreads` API could be used to implement a callback handling system. Again, this is currently only an idea, as we are not aware of anyone actually realizing such an implementation. Building a callback handling system on top of WASI already has its own challenges, but using it on embedded systems adds an additional layer of constraints. As a start, simply supporting the full WASI specification on embedded devices has proven to be complicated in practice @massey21. To the best of our knowledge there is no WebAssembly runtime for constrained devices that fully supports WASI. Moreover, this approach does not seem to have much traction in the community, as there are several online discussion threads to add a more dedicated API for asynchronous interrupts to WASI#footnote[In particular the #link(
